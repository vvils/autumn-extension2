...
### 2.5. Propagate prompt source from side panel to background

Currently all prompt types (user-typed, workflow Quick Actions, shortcuts) arrive at the background as identical `new_task` messages with fields `{ type, task, taskId, tabId }`. The `displayText` and `shortcutsMeta` parameters in `handleSendMessage` are UI-only — they never reach the background. To enable source-based enhancement routing, the `new_task` message must carry a `source` field.
...
Update `handleSendMessage` (line 932) to accept a `source` parameter:
...
const handleSendMessage = async (
  ...
  source?: 'user' | 'workflow' | 'shortcut',
) => {
```

After slash command expansion (line 943-951), if a shortcut was matched and `source` was not already set, override it:
...
Include `source` in the `new_task` message (line 1015-1020):
...
Update all call sites of `handleSendMessage`:

- **Workflow Quick Action buttons** (`SidePanel.tsx:1305`): `handleSendMessage(wp.prompt, wp.name, undefined, 'workflow')`

**File**: `pages/side-panel/src/components/ChatInput.tsx`

Update `ChatInputProps.onSendMessage` type (line 23):
...
Update call sites inside ChatInput:

- **Normal user submit** (`ChatInput.tsx:426`): `onSendMessage(messageContent, displayContent)` — no `source` arg, defaults to `'user'` in `handleSendMessage`
- **Shortcut execution** (`ChatInput.tsx:382`): `onSendMessage(messageContent, displayContent, [{ command, prompt }], 'shortcut')`
- **Quickstart workflow select** (`ChatInput.tsx:611`): `onSendMessage(wp.prompt, wp.name, undefined, 'workflow')`
...
- `new_task` handler (line 368): `const { executor, enhancedTask } = await setupExecutor(message.taskId, message.task, browserContext, { tabId: message.tabId, skipEnhancement: message.source !== 'user' });` — only user-typed prompts are enhanced; workflows and shortcuts pass through unchanged
...
- **Source field on `new_task` message** — the side panel passes `source: 'user' | 'workflow' | 'shortcut'` so the background can route enhancement decisions. This is explicit and future-proof (e.g., a future `'voice'` source could opt in/out independently). The alternative — inferring source from `displayText`/`shortcutsMeta` heuristics — is fragile because `displayText` is also set for file attachments in user-typed messages.
...
| `chrome-extension/src/background/index.ts` | Wire enhancer into `setupExecutor()` + both call sites (new_task with `message.source`, replay with `skipEnhancement: true`) + prompt_enhanced notification |
...
| `pages/side-panel/src/SidePanel.tsx` | Add `source` param to `handleSendMessage`, include in `new_task` message, handle `prompt_enhanced` port message |
| `pages/side-panel/src/components/ChatInput.tsx` | Update `onSendMessage` prop type to accept `source`, pass `'workflow'` from quickstart and `'shortcut'` from shortcut execution |
...
9. Manual test: click a workflow Quick Action (e.g., OTA Parity), verify the prompt is NOT enhanced (passes through unchanged)
10. Manual test: type a `/shortcut` command, verify the expanded prompt is NOT enhanced
