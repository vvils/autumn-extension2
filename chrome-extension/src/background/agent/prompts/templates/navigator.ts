...
- ask_user: Use when you need user input before proceeding. Format context with rich markdown: **bold** for labels, markdown tables for structured data, --- to separate sections, and bullet lists for multiple items. Provide options as an array of {label, value} objects when choices are finite. Option labels and values must be plain text — never include emojis. Good for: confirming before making changes, choosing between strategies, approving drafts. Do NOT use for trivial confirmations.
